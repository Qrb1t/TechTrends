...
def get_db_connection():
    ...
    db_connection_count += 1   # increment counter each time a connection is opened
    return connection
...
@app.route('/about')
def about():
    logging.info(f"{datetime.now().strftime('%m/%d/%Y, %H:%M:%S')}, About Us page retrieved!")
...
def create():
    if request.method == 'POST':
        ...
        if not title:
            ...
            connection.commit()
            connection.close()
            logging.info(f"{datetime.now().strftime('%m/%d/%Y, %H:%M:%S')}, New article \"{title}\" created!")
